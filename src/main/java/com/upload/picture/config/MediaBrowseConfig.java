 ...
 * 媒体预览/浏览目录配置
 *
 * 管理 iOS 客户端可预览和浏览的媒体根目录（图片、视频、实况照片）
 * 支持对根目录的增删改查操作
 * 配置持久化存储在: ~/FileUploadManager/config/media_roots_config.json
 *
 * 上传文件的存储路径配置，请查看
 * @see StorageConfig
 */
@Component
public class MediaBrowseConfig {
    
    private static final Logger logger = LoggerFactory.getLogger(MediaBrowseConfig.class);
    ...
    public MediaBrowseConfig() {
        ...
        logger.info("媒体预览目录配置初始化完成，配置文件路径: {}, 已加载 {} 个根目录", 
